...
sample = df_sample[["date", "product_id", "store_id", "quantity_sold", "demand_lag_1d"]].dropna().head(1000)
lag1_match = 0
for idx, row in sample.iterrows():
    past = df_sample[(df_sample["date"] == row["date"] - pd.Timedelta(days=1)) & 
                     (df_sample["product_id"] == row["product_id"]) & 
                     (df_sample["store_id"] == row["store_id"])]
    if not past.empty and abs(past.iloc[0]["quantity_sold"] - row["demand_lag_1d"]) < 1e-9:
        lag1_match += 1
...
sample2 = df_sample[["date", "product_id", "store_id", "quantity_sold", "demand_rolling_mean_7d"]].dropna().head(1000)
rolling_match = 0
for idx, row in sample2.iterrows():
    hist = df_sample[(df_sample["date"] < row["date"]) & 
                     (df_sample["product_id"] == row["product_id"]) & 
                     (df_sample["store_id"] == row["store_id"]) &
...
sample3 = df_sample[["date", "product_id", "store_id", "quantity_sold", "target_demand_1d"]].dropna().head(1000)
target_match = 0
for idx, row in sample3.iterrows():
    future = df_sample[(df_sample["date"] == row["date"] + pd.Timedelta(days=1)) & 
                       (df_sample["product_id"] == row["product_id"]) & 
                       (df_sample["store_id"] == row["store_id"])]
    if not future.empty and abs(future.iloc[0]["quantity_sold"] - row["target_demand_1d"]) < 1e-9:
        target_match += 1
...
feature_cols = [c for c in df.columns if c not in ["date", "product_id", "store_id", "category", "subcategory", "store_type", "city", "state", "supplier_id", "warehouse_id"]]
suspicious = []
for col in feature_cols:
    ...
    if pd.notna(corr_past) and pd.notna(corr_future) and abs(corr_future) > abs(corr_past) + 0.1:
...
critical_features = ["demand_lag_1d", "demand_rolling_mean_7d", "demand_rolling_std_7d", 
                     "stock_coverage_days", "target_demand_1d"]
nan_counts = {}
...
for check_name, value, threshold in leakage_checks:
    if isinstance(threshold, str):
        ...
        passed = value <= threshold if check_name in ["No suspicious future correlations", "No NaN in critical features"] else value >= threshold
