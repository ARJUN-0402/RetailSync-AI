...
fig.update_layout(
    title_text="Daily Sales Trend (Revenue vs Quantity)",
...
fig = px.bar(
    ...
    x="month",
    y="quantity_sold",
    title="Average Daily Quantity by Month",
    labels={"month": "Month", "quantity_sold": "Avg Quantity"},
    template="plotly_dark",
    color="quantity_sold",
...
dow_avg = (
    sales.groupby("day_of_week")["quantity_sold"]
    .mean()
    .reindex(
...
fig = px.bar(
    ...
    x="day_of_week",
    y="quantity_sold",
    title="Average Daily Quantity by Day of Week",
    labels={"day_of_week": "Day", "quantity_sold": "Avg Quantity"},
    template="plotly_dark",
    color="quantity_sold",
    color_continuous_scale="Viridis",
)
...
cat_month = (
    sales_with_cat.groupby(["category", "month"])["quantity_sold"].mean().reset_index()
)
cat_month_pivot = cat_month.pivot(
    index="category", columns="month", values="quantity_sold"
)
fig = px.imshow(
    cat_month_pivot,
    labels={"x": "Month", "y": "Category", "color": "Avg Quantity"},
    title="Category Seasonality Heatmap",
    template="plotly_dark",
...
store_perf = (
    ...
    .reset_index()
    .merge(
        stores[["store_id", "store_name", "city", "state", "store_type"]], on="store_id"
    )
)
...
fig = px.bar(
    ...
    labels={"store_type": "Store Type", "total_revenue": "Revenue ($)"},
    template="plotly_dark",
    color="store_type",
)
...
latest_inv = inventory.loc[
    inventory.groupby(["product_id", "store_id"])["date"].idxmax()
]
...
fig = px.histogram(
    ...
    x="quantity_on_hand",
    nbins=50,
    title="Distribution of Current Inventory Levels",
    labels={"quantity_on_hand": "Quantity On Hand"},
    template="plotly_dark",
    color="category",
)
...
fig = px.bar(
    ...
    x="category",
    y="avg_stock",
    title="Average Inventory Level by Category",
    labels={"category": "Category", "avg_stock": "Avg Stock"},
    template="plotly_dark",
    color="avg_stock",
    color_continuous_scale="Blues",
)
...
# ============================================================
print("\n=== 6. REVENUE TRENDS ===")
daily_sales["revenue_7d_ma"] = (
    daily_sales["total_revenue"].rolling(window=7, min_periods=1).mean()
)
fig = go.Figure()
fig.add_trace(
    go.Scatter(
...
fig.update_layout(
    title="Daily Revenue with 7-Day Moving Average", template="plotly_dark", height=500
)
...
promo = (
    ...
    .agg(total_revenue=("revenue", "sum"), total_quantity=("quantity_sold", "sum"))
    .reset_index()
)
...
fig = px.pie(
    promo,
    values="total_revenue",
    names="promotion_label",
    title="Revenue: Promotional vs Regular",
    template="plotly_dark",
)
...
print("\n=== 7. DEMAND VARIABILITY ===")
daily_product = (
    sales.groupby(["date", "product_id"])["quantity_sold"].sum().reset_index()
)
variability = (
    daily_product.groupby("product_id")["quantity_sold"]
    .agg(["mean", "std", "count"])
    .reset_index()
)
...
fig = px.histogram(
    ...
    x="cv",
    nbins=40,
    title="Distribution of Demand Variability (Coefficient of Variation)",
...
fig = px.bar(
    ...
    y="product_name",
    orientation="h",
    title="Top 10 Products by Demand Variability",
...
stockout_freq = (
    stockouts.groupby(["product_id", "store_id"])
    .size()
    .reset_index(name="stockout_count")
)
stockout_freq = stockout_freq.merge(
    products[["product_id", "category"]], on="product_id", how="left"
)
stockout_summary = (
    stockout_freq.groupby("product_id")
    .agg(
    ...
    .reset_index()
    .merge(products[["product_id", "product_name", "category"]], on="product_id")
    .sort_values("total_stockouts", ascending=False)
)

fig = px.bar(
    stockout_summary.head(15),
    x="total_stockouts",
    y="product_name",
    orientation="h",
    title="Top 15 Products by Stockout Frequency",
    labels={"total_stockouts": "Stockout Snapshots", "product_name": "Product"},
    template="plotly_dark",
    color="category",
)
...
fig = px.pie(
    stockout_cat,
    values="stockout_count",
    names="category",
    title="Stockout Distribution by Category",
    template="plotly_dark",
)
