...
print(f"  Products: {features['product_id'].nunique()}, Stores: {features['store_id'].nunique()}")
...
print(f"\nSegments:")
print(f"  Products: {len(product_segments)} ({product_segments['product_cluster_label'].value_counts().to_dict()})")
print(f"  Stores: {len(store_segments)} ({store_segments['store_cluster_label'].value_counts().to_dict()})")
print(f"  Warehouses: {len(warehouse_segments)} ({warehouse_segments['warehouse_cluster_label'].value_counts().to_dict()})")
...
insights = {
    ...
    "segmentation": {
        "product_clusters": {
            "k": 2,
            "silhouette": 0.234,
            "labels": product_segments["product_cluster_label"].value_counts().to_dict(),
        },
        "store_clusters": {
            "k": 2,
            "silhouette": 0.337,
        ...
        "warehouse_clusters": {
            "k": 4,
            "silhouette": 0.826,
            "labels": warehouse_segments["warehouse_cluster_label"].value_counts().to_dict(),
        },
    },
    "warehouse": {
...
summary_lines.append(f"2. **Inventory Intelligence** — Detect stockout, overstock, and dead stock risks")
summary_lines.append(f"3. **Anomaly Detection** — Identify unusual demand spikes and patterns")
summary_lines.append(f"4. **Segmentation** — Group products, stores, and warehouses by behavior")
summary_lines.append(f"5. **Warehouse Optimization** — Analyze utilization and capacity")
summary_lines.append(f"\n## Data")
summary_lines.append(f"\n- **Source:** Synthetic retail data")
summary_lines.append(f"- **Date range:** 2023-08-11 to 2025-08-09 (730 days)")
summary_lines.append(f"- **Products:** 50")
summary_lines.append(f"- **Stores:** 10")
summary_lines.append(f"- **Warehouses:** 5")
summary_lines.append(f"- **Sales records:** 69,216")
summary_lines.append(f"- **Inventory records:** 52,500")
summary_lines.append(f"\n## Key Results")
summary_lines.append(f"\n### Demand Forecasting")
...
summary_lines.append(f"- **14-day forecast:** {insights['forecasting']['total_forecasted_demand']:,} units, ${insights['forecasting']['total_forecasted_revenue']:,.2f}")
...
summary_lines.append(f"- **Stockout MEDIUM:** {insights['inventory']['stockout_medium']}")
summary_lines.append(f"- **Overstock HIGH:** {insights['inventory']['overstock_high']}")
summary_lines.append(f"- **Urgent reorder needed:** {insights['inventory']['urgent_reorder']}")
summary_lines.append(f"\n### Anomaly Detection")
...
summary_lines.append(f"- **Unusual patterns:** {insights['anomalies']['unusual_patterns']:,}")
summary_lines.append(f"\n### Segmentation")
summary_lines.append(f"\n- **Product clusters (K=2):** Silhouette={insights['segmentation']['product_clusters']['silhouette']}")
summary_lines.append(f"  - Labels: {insights['segmentation']['product_clusters']['labels']}")
summary_lines.append(f"- **Store clusters (K=2):** Silhouette={insights['segmentation']['store_clusters']['silhouette']}")
summary_lines.append(f"  - Labels: {insights['segmentation']['store_clusters']['labels']}")
summary_lines.append(f"- **Warehouse clusters (K=4):** Silhouette={insights['segmentation']['warehouse_clusters']['silhouette']}")
summary_lines.append(f"  - Labels: {insights['segmentation']['warehouse_clusters']['labels']}")
summary_lines.append(f"\n### Warehouse Optimization")
summary_lines.append(f"\n- **Warehouses analyzed:** {insights['warehouse']['total_warehouses']}")
summary_lines.append(f"- **Total capacity:** {insights['warehouse']['total_capacity_m3']:,.0f} m³")
summary_lines.append(f"- **Average utilization:** {insights['warehouse']['avg_utilization_pct']:.1f}%")
summary_lines.append(f"- **High utilization (>80%):** {insights['warehouse']['high_utilization']}")
summary_lines.append(f"- **Low utilization (<50%):** {insights['warehouse']['low_utilization']}")
summary_lines.append(f"\n## Pipeline Architecture")
summary_lines.append(f"\n```")
summary_lines.append(f"Raw Data -> Cleaning -> Feature Engineering -> Models -> Predictions -> Risk Detection -> Business Insights")
summary_lines.append(f"```")
summary_lines.append(f"\n## Files and Outputs")
summary_lines.append(f"\n| Output | File | Rows/Records |")
summary_lines.append(f"|--------|------|--------------|")
summary_lines.append(f"| Features | `data/processed/features_daily.csv` | {features.shape[0]:,} |")
summary_lines.append(f"| Forecasts | `data/processed/forecasts_next_14d.csv` | {forecasts.shape[0]:,} |")
summary_lines.append(f"| Inventory Alerts | `data/processed/inventory_intelligence.csv` | {len(inv_intel):,} |")
summary_lines.append(f"| Anomalies | `data/processed/anomalies.csv` | {len(anomalies):,} |")
summary_lines.append(f"| Product Segments | `data/processed/product_segments.csv` | {len(product_segments)} |")
summary_lines.append(f"| Store Segments | `data/processed/store_segments.csv` | {len(store_segments)} |")
summary_lines.append(f"| Warehouse Segments | `data/processed/warehouse_segments.csv` | {len(warehouse_segments)} |")
summary_lines.append(f"| Warehouse Optimization | `data/processed/warehouse_optimization.csv` | {len(wh_opt)} |")
summary_lines.append(f"\n## Model Artifacts")
summary_lines.append(f"\n| Model | File | Type |")
summary_lines.append(f"|-------|------|------|")
summary_lines.append(f"| Demand Forecaster | `models/demand_forecaster.pkl` | Baseline Mean |")
summary_lines.append(f"| Product Clusterer | `models/product_clusterer.pkl` | K-Means (K=2) |")
summary_lines.append(f"| Store Clusterer | `models/store_clusterer.pkl` | K-Means (K=2) |")
summary_lines.append(f"| Warehouse Clusterer | `models/warehouse_clusterer.pkl` | K-Means (K=4) |")
summary_lines.append(f"\n## How to Run")
summary_lines.append(f"\n```bash")
summary_lines.append(f"# Run individual components")
summary_lines.append(f"python src/data/generate_dataset.py")
summary_lines.append(f"python src/data/ingest.py")
summary_lines.append(f"python src/database/init_db.py")
summary_lines.append(f"python src/features/feature_engineering.py")
summary_lines.append(f"python src/forecasting/demand_forecaster.py")
summary_lines.append(f"python src/forecasting/forecast_pipeline.py")
summary_lines.append(f"python src/inventory/inventory_intelligence.py")
summary_lines.append(f"python src/anomaly/anomaly_detection.py")
summary_lines.append(f"python src/clustering/segmentation.py")
summary_lines.append(f"python src/clustering/warehouse_optimization.py")
summary_lines.append(f"```")
summary_lines.append(f"\n## Limitations")
summary_lines.append(f"\n1. **Synthetic data:** Results are based on synthetic data, not real retail operations.")
summary_lines.append(f"2. **High zero-inflation:** 81% of demand values are zero, making forecasting challenging.")
summary_lines.append(f"3. **Baseline forecaster:** The best forecasting model is a simple mean due to data sparsity.")
summary_lines.append(f"4. **Static analysis:** Clustering and optimization are based on historical snapshots.")
summary_lines.append(f"\n## Next Steps")
summary_lines.append(f"\n- Dashboard development (Phase 12)")
summary_lines.append(f"- Testing and validation (Phase 14)")
summary_lines.append(f"- Deployment (Phase 16)")
summary_lines.append(f"- GitHub portfolio preparation (Phase 17)")
...
print(f"Pipeline summary saved to docs/pipeline_summary.md")
