...
holiday_lift = cat_props.map(lambda x: x["holiday_lift"])
...
print(f"Zero demand pct: {(sales['quantity_sold'] == 0).mean() * 100:.2f}%")
...
print(f"\nGenerated raw data in {OUTPUT_DIR}/")
...
print(f"Zero demand pct: {(sales['quantity_sold'] == 0).mean() * 100:.2f}%")
print(f"Avg daily demand per product-store: {sales['quantity_sold'].mean():.2f}")
