...
products = pd.DataFrame({
    ...
    "supplier_id": np.random.choice([f"S{str(i).zfill(2)}" for i in range(1, NUM_SUPPLIERS + 1)], NUM_PRODUCTS),
...
stores = pd.DataFrame({
    ...
    "city": np.random.choice(["New York", "Los Angeles", "Chicago", "Houston", "Phoenix", "Philadelphia", "San Antonio", "San Diego", "Dallas", "San Jose"], NUM_STORES),
    "state": np.random.choice(["NY", "CA", "IL", "TX", "AZ", "PA", "TX", "CA", "TX", "CA"], NUM_STORES),
...
suppliers = pd.DataFrame({
    ...
    "country": np.random.choice(["USA", "China", "Germany", "Japan", "Mexico", "Vietnam", "India", "UK"], NUM_SUPPLIERS),
...
warehouses = pd.DataFrame({
    ...
    "city": np.random.choice(["Dallas", "Chicago", "Los Angeles", "Atlanta", "Seattle"], NUM_WAREHOUSES),
    "state": np.random.choice(["TX", "IL", "CA", "GA", "WA"], NUM_WAREHOUSES),
    "capacity_m3": np.random.randint(5000, 20000, NUM_WAREHOUSES),
    "supplier_id": np.random.choice([f"S{str(i).zfill(2)}" for i in range(1, NUM_SUPPLIERS + 1)], NUM_WAREHOUSES),
})
...
for date in dates:
    ...
    for store_id in stores["store_id"]:
        ...
        for product_id in sampled_products:
            ...
            seasonal_factor = 1.3 if product["category"] in ["Electronics"] and month in [11, 12] else 1.2 if product["category"] == "Clothing" and month in [6, 7, 8] else 1.0
