...
all_products_stores = sales[["product_id", "store_id"]].drop_duplicates().reset_index(drop=True)
full_dates = pd.date_range(start=sales["date"].min(), end=sales["date"].max(), freq="D")

print(f"Dates: {len(full_dates)}, Product-Stores: {len(all_products_stores)}, Total rows: {len(full_dates) * len(all_products_stores):,}")
...
daily_base = all_products_stores.assign(key=1).merge(
...
daily_df = daily_base.merge(daily_sales, on=["date", "product_id", "store_id"], how="left")
...
daily_df = daily_df.merge(products[["product_id", "category", "subcategory", "cost_price", "supplier_id", "weight_kg", "volume_m3"]], on="product_id", how="left")
daily_df = daily_df.merge(stores[["store_id", "store_type", "city", "state"]], on="store_id", how="left")
daily_df = daily_df.merge(suppliers[["supplier_id", "lead_time_days", "reliability_score"]], on="supplier_id", how="left")
...
for window in [7, 14, 28]:
    rolling = qty_shifted.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False)
    daily_df[f"demand_rolling_mean_{window}d"] = rolling.rolling(window=window, min_periods=1).mean().reset_index(level=[0,1], drop=True)
    daily_df[f"demand_rolling_std_{window}d"] = rolling.rolling(window=window, min_periods=1).std().reset_index(level=[0,1], drop=True).fillna(0)
    daily_df[f"demand_rolling_max_{window}d"] = rolling.rolling(window=window, min_periods=1).max().reset_index(level=[0,1], drop=True)
    daily_df[f"demand_rolling_min_{window}d"] = rolling.rolling(window=window, min_periods=1).min().reset_index(level=[0,1], drop=True)

# Expanding features
daily_df["demand_expanding_mean"] = qty_shifted.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False).expanding(min_periods=1).mean().reset_index(level=[0,1], drop=True)
daily_df["demand_expanding_std"] = qty_shifted.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False).expanding(min_periods=1).std().reset_index(level=[0,1], drop=True).fillna(0)
...
daily_df["price_margin_pct"] = (daily_df["price_vs_cost"] / daily_df["cost_price"].replace(0, np.nan)).fillna(0)

promo_shifted = daily_df.groupby(["product_id", "store_id"])["promotion"].shift(1)
daily_df["promotion_last_7d"] = promo_shifted.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False).rolling(window=7, min_periods=1).sum().reset_index(level=[0,1], drop=True)
daily_df["promotion_last_14d"] = promo_shifted.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False).rolling(window=14, min_periods=1).sum().reset_index(level=[0,1], drop=True)
...
inv_sorted = inventory[["date", "product_id", "store_id", "quantity_on_hand", "reorder_point", "max_stock_level", "warehouse_id"]].sort_values(["product_id", "store_id", "date"])
...
daily_df["warehouse_id"] = daily_df.groupby(["product_id", "store_id"])["warehouse_id"].ffill().fillna("Unknown")
...
for horizon in [1, 7, 14]:
    daily_df[f"target_demand_{horizon}d"] = daily_df.groupby(["product_id", "store_id"])["quantity_sold"].shift(-horizon)
    daily_df[f"target_revenue_{horizon}d"] = daily_df.groupby(["product_id", "store_id"])["revenue"].shift(-horizon)
...
qty_for_cv = qty_shifted.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False)
rolling_mean_28 = qty_for_cv.rolling(window=28, min_periods=7).mean().reset_index(level=[0,1], drop=True)
rolling_std_28 = qty_for_cv.rolling(window=28, min_periods=7).std().reset_index(level=[0,1], drop=True)
daily_df["demand_cv_28d"] = np.where(rolling_mean_28 > 0, rolling_std_28 / rolling_mean_28, 0)
daily_df["demand_cv_28d"] = daily_df["demand_cv_28d"].fillna(0)

# Zero-demand proportion
zero_mask = (qty_shifted == 0).astype(float)
daily_df["zero_demand_pct_28d"] = zero_mask.groupby([daily_df["product_id"], daily_df["store_id"]], group_keys=False).rolling(window=28, min_periods=7).mean().reset_index(level=[0,1], drop=True).fillna(0)
...
daily_df = daily_df.dropna(subset=["target_demand_1d", "target_demand_7d", "target_demand_14d"]).reset_index(drop=True)
...
print(f"  Time features: {sum(1 for c in daily_df.columns if c in ['day_of_week', 'day_of_month', 'month', 'quarter', 'year', 'is_weekend', 'is_month_start', 'is_month_end', 'month_sin', 'month_cos', 'dow_sin', 'dow_cos'])}")
...
print(f"  Inventory features: {sum(1 for c in daily_df.columns if any(x in c for x in ['stock', 'inventory', 'reorder', 'coverage']))}")
print(f"  Demand variability: {sum(1 for c in daily_df.columns if 'cv' in c or 'zero' in c)}")
print(f"  Aggregate features: {sum(1 for c in daily_df.columns if 'category_avg' in c or 'store_type_avg' in c)}")
