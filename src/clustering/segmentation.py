...
product_features.columns = ["product_id"] + [f"{col[0]}_{col[1]}" for col in product_features.columns[1:]]

# Add static attributes
product_static = df[["product_id", "category", "subcategory", "weight_kg", "volume_m3"]].drop_duplicates()
product_features = product_features.merge(product_static, on="product_id", how="left")

# Add inventory metrics
product_inv = df.groupby("product_id").agg({
...
product_cluster_features = [
    ...
    "revenue_sum", "revenue_mean", "demand_cv_28d_mean", "zero_demand_pct_28d_mean",
    "stock_coverage_days_mean", "inv_quantity_on_hand", "inv_reorder_point",
    "inv_max_stock_level", "unit_price_mean", "cost_price_mean"
]

# Handle NaN and infinite values
product_features[product_cluster_features] = product_features[product_cluster_features].fillna(0)
product_features[product_cluster_features] = product_features[product_cluster_features].replace([np.inf, -np.inf], 0)
...
plot_elbow(range(2, 8), inertias_prod, silhouettes_prod, "Product Segmentation", "docs/product_cluster_elbow.png")
...
print(f"  Saved: models/product_clusterer.pkl")
...
store_features.columns = ["store_id"] + [f"{col[0]}_{col[1]}" for col in store_features.columns[1:]]
...
# Add inventory metrics
store_inv = df.groupby("store_id").agg({
    "quantity_on_hand": "mean",
    "reorder_point": "mean",
    "max_stock_level": "mean",
}).reset_index()
...
store_cluster_features = [
    ...
    "revenue_sum", "revenue_mean", "demand_cv_28d_mean", "zero_demand_pct_28d_mean",
    "stock_coverage_days_mean", "inv_quantity_on_hand", "inv_reorder_point",
    "inv_max_stock_level", "unit_price_mean"
]

# Handle NaN
store_features[store_cluster_features] = store_features[store_cluster_features].fillna(0)
store_features[store_cluster_features] = store_features[store_cluster_features].replace([np.inf, -np.inf], 0)
...
plot_elbow(range(2, 6), inertias_store, silhouettes_store, "Store Segmentation", "docs/store_cluster_elbow.png")
...
joblib.dump({"model": kmeans_store, "scaler": scaler_store, "features": store_cluster_features}, "models/store_clusterer.pkl")
print(f"  Saved: models/store_clusterer.pkl")
...
warehouse_features = df.groupby("warehouse_id").agg({
    "quantity_sold": ["sum", "mean", "std"],
    "revenue": ["sum", "mean"],
    "demand_cv_28d": "mean",
    "stock_coverage_days": "mean",
...
warehouse_features.columns = ["warehouse_id"] + [f"{col[0]}_{col[1]}" for col in warehouse_features.columns[1:]]
...
warehouse_cluster_features = [
    "quantity_sold_sum", "quantity_sold_mean", "quantity_sold_std",
    "revenue_sum", "revenue_mean", "demand_cv_28d_mean", "stock_coverage_days_mean",
    "quantity_on_hand_mean", "reorder_point_mean", "max_stock_level_mean"
]

# Handle NaN
warehouse_features[warehouse_cluster_features] = warehouse_features[warehouse_cluster_features].fillna(0)
warehouse_features[warehouse_cluster_features] = warehouse_features[warehouse_cluster_features].replace([np.inf, -np.inf], 0)
...
plot_elbow(range(2, 5), inertias_wh, silhouettes_wh, "Warehouse Segmentation", "docs/warehouse_cluster_elbow.png")
...
joblib.dump({"model": kmeans_wh, "scaler": scaler_wh, "features": warehouse_cluster_features}, "models/warehouse_clusterer.pkl")
print(f"  Saved: models/warehouse_clusterer.pkl")
...
# Product clusters
print("\n--- Product Clusters ---")
product_cluster_stats = interpret_clusters(product_features, "product_cluster", product_cluster_features, "Product")

# Store clusters
print("\n--- Store Clusters ---")
store_cluster_stats = interpret_clusters(store_features, "store_cluster", store_cluster_features, "Store")

# Warehouse clusters
print("\n--- Warehouse Clusters ---")
warehouse_cluster_stats = interpret_clusters(warehouse_features, "warehouse_cluster", warehouse_cluster_features, "Warehouse")
...
def label_product_cluster(row):
    if row["revenue_sum"] > product_features["revenue_sum"].quantile(0.75) and row["demand_cv_28d_mean"] < product_features["demand_cv_28d_mean"].median():
        return "High-Volume / Stable"
    elif row["revenue_sum"] > product_features["revenue_sum"].quantile(0.75) and row["demand_cv_28d_mean"] >= product_features["demand_cv_28d_mean"].median():
        return "High-Volume / Volatile"
    elif row["revenue_sum"] <= product_features["revenue_sum"].quantile(0.25) and row["demand_cv_28d_mean"] >= product_features["demand_cv_28d_mean"].median():
...
product_features["product_cluster_label"] = product_features.apply(label_product_cluster, axis=1)
...
store_features["store_cluster_label"] = store_features.apply(label_store_cluster, axis=1)
...
def label_warehouse_cluster(row):
    ...
    elif row["stock_coverage_days_mean"] > warehouse_features["stock_coverage_days_mean"].median():
        return "Overstocked"
    elif row["stock_coverage_days_mean"] < warehouse_features["stock_coverage_days_mean"].quantile(0.25):
        return "Underutilized"
    else:
        return "Balanced"

warehouse_features["warehouse_cluster_label"] = warehouse_features.apply(label_warehouse_cluster, axis=1)
...
print(f"Saved: data/processed/warehouse_segments.csv ({len(warehouse_features)} warehouses)")
...
df_with_segments = df.merge(
    ...
    warehouse_features[["warehouse_id", "warehouse_cluster", "warehouse_cluster_label"]],
    on="warehouse_id", how="left"
)

df_with_segments.to_csv("data/processed/features_with_segments.csv", index=False)
print(f"Saved: data/processed/features_with_segments.csv ({len(df_with_segments)} rows)")
...
with engine.connect() as conn:
    ...
    # Load new segments
    prod_insert = product_features[["product_id", "product_cluster", "product_cluster_label", "revenue_sum", "demand_cv_28d_mean"]].rename(
        columns={"product_cluster": "cluster", "product_cluster_label": "cluster_label"}
    )
    store_insert = store_features[["store_id", "store_cluster", "store_cluster_label", "revenue_sum", "demand_cv_28d_mean"]].rename(
        columns={"store_cluster": "cluster", "store_cluster_label": "cluster_label"}
    )
    wh_insert = warehouse_features[["warehouse_id", "warehouse_cluster", "warehouse_cluster_label", "revenue_sum", "stock_coverage_days_mean"]].rename(
        columns={"warehouse_cluster": "cluster", "warehouse_cluster_label": "cluster_label"}
...
# Verify
with engine.connect() as conn:
    prod_count = conn.execute(text("SELECT COUNT(*) FROM product_segments")).fetchone()[0]
    store_count = conn.execute(text("SELECT COUNT(*) FROM store_segments")).fetchone()[0]
    wh_count = conn.execute(text("SELECT COUNT(*) FROM warehouse_segments")).fetchone()[0]
