...
warehouse_trends = (
    inventory.groupby(["warehouse_id", "date"])
    .agg(
...
warehouse_util = warehouse_util.merge(
    warehouse_segments[["warehouse_id", "cluster_label"]], on="warehouse_id", how="left"
)

print("\nWarehouse Clusters:")
print(
    warehouse_util[
        ["warehouse_id", "cluster_label", "utilization_pct", "capacity_risk"]
    ].to_string(index=False)
)
...
warehouse_util.loc[high_util_mask, "recommendation"] = (
    "Expand capacity or redistribute inventory"
)
warehouse_util.loc[high_util_mask, "optimization_potential"] = (
    warehouse_util.loc[high_util_mask, "utilization_pct"] - 80
) / 100

# Low utilization - underutilized
low_util_mask = warehouse_util["utilization_pct"] < 50
warehouse_util.loc[low_util_mask, "recommendation"] = (
    "Consolidate inventory or reduce footprint"
)
warehouse_util.loc[low_util_mask, "optimization_potential"] = (
    50 - warehouse_util.loc[low_util_mask, "utilization_pct"]
) / 100
...
warehouse_util.loc[
    warehouse_util["cluster_label"] == "High-Utilization", "recommendation"
] = "Consider expansion or overflow to other warehouses"
warehouse_util.loc[
    warehouse_util["cluster_label"] == "Underutilized", "recommendation"
] = "Redirect inventory from high-utilization warehouses"
warehouse_util.loc[
    warehouse_util["cluster_label"] == "Overstocked", "recommendation"
] = "Run promotions or reduce incoming orders"
warehouse_util.loc[warehouse_util["cluster_label"] == "Balanced", "recommendation"] = (
...
for _, row in warehouse_util.iterrows():
    ...
    print(
        f"    Utilization: {row['utilization_pct']:.1f}% | Capacity Risk: {row['capacity_risk']}"
    )
...
print(
    f"Saved: data/processed/warehouse_optimization.csv ({len(warehouse_util)} warehouses)"
)
...
summary = {
    ...
    "underutilized_warehouses": (
        warehouse_util["cluster_label"] == "Underutilized"
    ).sum(),
    "high_utilization_warehouses": (
        warehouse_util["cluster_label"] == "High-Utilization"
    ).sum(),
}

summary_df = pd.DataFrame([summary])
summary_df.to_csv("data/processed/warehouse_optimization_summary.csv", index=False)
print("Saved: data/processed/warehouse_optimization_summary.csv")
...
with engine.connect() as conn:
    ...
    conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS idx_wh_opt_id ON warehouse_optimization(warehouse_id)"
        )
    )
    conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS idx_wh_opt_risk ON warehouse_optimization(capacity_risk)"
        )
    )
    conn.commit()
    ...
    warehouse_util[cols_for_db].to_sql(
        "warehouse_optimization", con=engine, if_exists="append", index=False
    )

    count = conn.execute(
        text("SELECT COUNT(*) FROM warehouse_optimization")
    ).fetchone()[0]
