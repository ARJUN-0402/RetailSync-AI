...
medium_stockout = (latest_inventory["quantity_on_hand"] > 0) & (latest_inventory["quantity_on_hand"] <= latest_inventory["reorder_point"])
...
latest_inventory = latest_inventory.merge(
    features_df.groupby(["product_id", "store_id"]).agg(
...
forecast_high = latest_inventory["forecast_demand_7d"] > latest_inventory["quantity_on_hand"]
latest_inventory.loc[forecast_high & (latest_inventory["stockout_risk"] == "LOW"), "stockout_risk"] = "MEDIUM"
latest_inventory.loc[forecast_high & (latest_inventory["stockout_risk"] == "MEDIUM"), "stockout_risk"] = "HIGH"
latest_inventory.loc[forecast_high, "stockout_reason"] = latest_inventory.loc[forecast_high, "stockout_reason"] + " + High forecasted demand"
...
high_overstock = latest_inventory["quantity_on_hand"] > latest_inventory["max_stock_level"] * 1.5
medium_overstock = (latest_inventory["quantity_on_hand"] > latest_inventory["max_stock_level"]) & (latest_inventory["quantity_on_hand"] <= latest_inventory["max_stock_level"] * 1.5)
low_overstock = latest_inventory["quantity_on_hand"] <= latest_inventory["max_stock_level"]
...
medium_cv = (latest_inventory["demand_cv_28d"] > 2.0) & (latest_inventory["demand_cv_28d"] <= 3.0)
latest_inventory.loc[high_cv & (latest_inventory["overstock_risk"] == "MEDIUM"), "overstock_risk"] = "HIGH"
latest_inventory.loc[high_cv & (latest_inventory["overstock_risk"] == "LOW"), "overstock_risk"] = "HIGH"
latest_inventory.loc[medium_cv & (latest_inventory["overstock_risk"] == "LOW"), "overstock_risk"] = "MEDIUM"

print(f"Overstock risk distribution:")
...
latest_inventory.loc[dead_condition, "dead_stock_reason"] = "High inventory with no demand"

# Also flag if quantity_on_hand > 0 but no sales in last 28 days
no_sales_28d = features_df.groupby(["product_id", "store_id"]).agg(
    sales_last_28d=("quantity_sold", lambda x: x.iloc[-28:].sum() if len(x) >= 28 else x.sum())
).reset_index()

latest_inventory = latest_inventory.merge(no_sales_28d, on=["product_id", "store_id"], how="left")
no_recent_sales = (latest_inventory["sales_last_28d"] == 0) & (latest_inventory["quantity_on_hand"] > 0)
...
print(f"Dead stock items: {dead_stock_count} ({dead_stock_count / len(latest_inventory) * 100:.1f}%)")
...
latest_inventory["stock_coverage_days"] = latest_inventory["stock_coverage_days"].replace([np.inf, -np.inf], 999).fillna(999)

# Reorder urgency
latest_inventory["reorder_urgency"] = "NONE"
latest_inventory["reorder_urgency_reason"] = ""

urgent_reorder = latest_inventory["quantity_on_hand"] <= latest_inventory["reorder_point"]
soon_reorder = (latest_inventory["quantity_on_hand"] > latest_inventory["reorder_point"]) & (latest_inventory["stock_coverage_days"] <= 7)
monitor_reorder = (latest_inventory["quantity_on_hand"] > latest_inventory["reorder_point"]) & (latest_inventory["stock_coverage_days"] <= 14)
...
print(f"Reorder urgency distribution:")
...
latest_inventory["stockout_score"] = latest_inventory["stockout_risk"].map({"HIGH": 100, "MEDIUM": 60, "LOW": 20})
latest_inventory["overstock_score"] = latest_inventory["overstock_risk"].map({"HIGH": 100, "MEDIUM": 60, "LOW": 20})
latest_inventory["dead_stock_score"] = latest_inventory["dead_stock"].map({True: 100, False: 0})
latest_inventory["reorder_score"] = latest_inventory["reorder_urgency"].map({"URGENT": 100, "SOON": 70, "MONITOR": 40, "NONE": 0})
...
print(f"Composite risk distribution:")
...
latest_inventory.loc[latest_inventory["stockout_risk"] == "HIGH", "recommended_action"] = "Immediate restock required"
latest_inventory.loc[latest_inventory["stockout_risk"] == "MEDIUM", "recommended_action"] = "Schedule restock soon"

# Overstock recommendations
latest_inventory.loc[latest_inventory["overstock_risk"] == "HIGH", "recommended_action"] = "Run promotion or reduce orders"
latest_inventory.loc[latest_inventory["overstock_risk"] == "MEDIUM", "recommended_action"] = "Review upcoming demand"

# Dead stock recommendations
latest_inventory.loc[latest_inventory["dead_stock"] == True, "recommended_action"] = "Consider clearance or return to supplier"

# Reorder recommendations
latest_inventory.loc[latest_inventory["reorder_urgency"] == "URGENT", "recommended_action"] = "Place emergency order"
latest_inventory.loc[latest_inventory["reorder_urgency"] == "SOON", "recommended_action"] = "Place order within 7 days"
...
print(f"Saved: data/processed/inventory_intelligence.csv ({len(latest_inventory)} rows)")
...
print(f"Saved: data/processed/inventory_intelligence_summary.csv")
...
critical_items = latest_inventory[
    latest_inventory["composite_risk_level"].isin(["CRITICAL", "HIGH"])
].sort_values("composite_risk_score", ascending=False).head(10)

print(critical_items[["product_id", "store_id", "stockout_risk", "overstock_risk", "dead_stock", "reorder_urgency", "composite_risk_score", "recommended_action"]].to_string(index=False))
